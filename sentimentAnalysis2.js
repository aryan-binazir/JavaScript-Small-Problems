...
function sentiment(text) {
  let customPositiveArr = [];
  let customNegativeArr = [];

  let wordList = text.toLowerCase().match(/[a-z']+/g);

  wordList.forEach(word => {
    if (word.match(positiveRegex)) {
      customPositiveArr.push(word);
    } else if (word.match(negativeRegex)) {
      customNegativeArr.push(word);
    }
  });
  processSentiment(customPositiveArr, customNegativeArr);
}

function processSentiment(customPositiveArr, customNegativeArr) {
  let numPositives = customPositiveArr.length;
  let numNegatives = customNegativeArr.length;

  let sentiment;

  if (numPositives > numNegatives) {
    sentiment = 'Positive';
  } else if (numPositives < numNegatives) {
    sentiment = 'Negative';
  } else {
    sentiment = 'Neutral';
  }

  console.log(`There are ${numPositives} positive words in the text.`);
  console.log(`Positive sentiments: ${customPositiveArr.join(', ')}`);
  console.log('\n');
  console.log(`There are ${numNegatives} negative words in the text.`);
  console.log(`Negative sentiments: ${customNegativeArr.join(', ')}`);
  console.log('\n');
