...
function mailCount(emailData) {
  ...
  emails.forEach(email => {
    email.split('#/#').forEach(section => {
      if (section.match(/Date/)) {
        let currentDate = section.match(/\d\d-\d\d-\d\d\d\d/)[0];
        emailDates.push(convertToDate(currentDate));
      }
    });
  });
  let dateRange = getDateRange(emailDates);
...
function getDateRange(emailDates) {
  let dateObj = [...emailDates].sort((a,b) => a.valueOf() - b.valueOf());
  return dateObj[0].toDateString() + ' - ' + dateObj[dateObj.length - 1].toDateString();
}
